...
import { type ContactMessage, type ActionResponse } from "./types"
 ...
 */
export async function sendMessage(formData: FormData): Promise<ActionResponse> {
    try {
        await initDB()
        ...
        const [rateLimitRows] = await pool.query<({ count: number })[] & RowDataPacket[]>(
 ...
 */
export async function updateMessageStatus(id: number, status: string): Promise<ActionResponse> {
    try {
        await requireAdmin()
        await initDB()
        await pool.query(
