...
import { validateData, DungeonSetSchema, type DungeonSet as DungeonSetInput } from './validation'
import { type DungeonSet, type ActionResponse } from './types'
...
export async function getDungeons() {
    ...
    const [rows] = await pool.query<({ dungeon_key: string; set_count: number })[] & RowDataPacket[]>(
        'SELECT dungeon_key, COUNT(*) as set_count FROM dungeon_sets GROUP BY dungeon_key'
    )
    const countMap = new Map(rows.map(r => [r.dungeon_key, Number(r.set_count)]))
...
export async function getDungeonInfo(dungeonKey: string) {
    return DUNGEON_ORDER.find(d => d.key === dungeonKey) || null
}

export async function getSetsByDungeon(dungeonKey: string) {
    await initDB()
    const [rows] = await pool.query<DungeonSet[] & RowDataPacket[]>(
...
export async function createSet(data: DungeonSetInput): Promise<ActionResponse> {
    await requireAdmin()
...
export async function updateSet(id: number, data: DungeonSetInput): Promise<ActionResponse> {
    await requireAdmin()
    ...
    try {
        ...
        const [rows] = await pool.query<DungeonSet[]>('SELECT dungeon_key FROM dungeon_sets WHERE id = ?', [id]);
        if (rows.length > 0) {
