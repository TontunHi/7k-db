...
export async function getRaids() {
    ...
    const [rows] = await pool.query<({ raid_key: string; set_count: number })[] & RowDataPacket[]>(
        'SELECT raid_key, COUNT(*) as set_count FROM raid_sets GROUP BY raid_key'
    )
    const countMap = new Map(rows.map(r => [r.raid_key, Number(r.set_count)]))
...
export async function getRaidInfo(raidKey: string) {
...
export async function createSet(data: RaidSetInput): Promise<ActionResponse> {
    await requireAdmin()
...
export async function updateSet(id: number, data: RaidSetInput): Promise<ActionResponse> {
    await requireAdmin()
    ...
    try {
        ...
        const [rows] = await pool.query<RaidSet[]>('SELECT raid_key FROM raid_sets WHERE id = ?', [id]);
        if (rows.length > 0) {
