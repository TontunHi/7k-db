...
import { validateData, TotalWarSetSchema, TotalWarTeamSchema, type TotalWarSetInput, type TotalWarTeamInput } from './validation'
import { type TotalWarSet, type TotalWarTeam, type ActionResponse } from './types'
import { type ResultSetHeader, type RowDataPacket } from 'mysql2'

// ─── Sets ────────────────────────────────────────────────────────────────────

/** Get all sets for a given tier, including their teams */
export async function getSetsByTier(tier: string) {
    await initDB()
...
export async function getAllSetCounts() {
    await initDB()
    const [rows] = await pool.query<({ tier: string; set_count: number })[] & RowDataPacket[]>(
...
export async function createSet(data: TotalWarSetInput): Promise<ActionResponse> {
    await requireAdmin()
...
export async function updateSet(id: number, data: Partial<TotalWarSetInput>): Promise<ActionResponse> {
    await requireAdmin()
    ...
    try {
        ...
        const [rows] = await pool.query<TotalWarSet[]>('SELECT tier FROM total_war_sets WHERE id = ?', [id])
...
export async function createTeam(data: TotalWarTeamInput): Promise<ActionResponse> {
    await requireAdmin()
...
export async function updateTeam(id: number, data: Partial<TotalWarTeamInput>): Promise<ActionResponse> {
    await requireAdmin()
...
export async function deleteTeam(id: number): Promise<ActionResponse> {
    await requireAdmin()
    try {
