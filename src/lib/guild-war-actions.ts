...
import { validateData, GuildWarTeamSchema, type GuildWarTeam as GuildWarTeamInput } from './validation'
import { type GuildWarTeam, type ActionResponse } from './types'
...
export async function getGuildWarTeams(type = 'attacker') {
    ...
    const [rows] = await pool.query<GuildWarTeam[]>(query, params)
...
export async function createGuildWarTeam(data: GuildWarTeamInput): Promise<ActionResponse> {
    await requireAdmin()
...
export async function updateGuildWarTeam(id: number, data: GuildWarTeamInput): Promise<ActionResponse> {
    await requireAdmin()
...
export async function getItemFiles() {
    try {
        const [rows] = await pool.query<({ filename: string; weapon_group: string; item_type: string })[] & RowDataPacket[]>(
            'SELECT image as filename, weapon_group, item_type FROM items'
        )
...
export async function deleteGuildWarTeam(id: number): Promise<ActionResponse> {
    await requireAdmin()
    try {
...
export async function reorderGuildWarTeams(orderedIds: number[]): Promise<ActionResponse> {
    await requireAdmin()
    try {
