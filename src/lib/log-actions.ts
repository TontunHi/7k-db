...
import { type SiteUpdate, type ActionResponse } from './types'
import { type RowDataPacket } from 'mysql2'

/** Get update logs for the site */
export async function getRecentUpdates(limit = 10) {
    await initDB()
    const [rows] = await pool.query<SiteUpdate[] & RowDataPacket[]>(
...
export async function getPaginatedUpdates({ type = 'all', page = 1, limit = 50 } = {}) {
    ...
    const [rows] = await pool.query<SiteUpdate[] & RowDataPacket[]>(query, params)
    const [countResult] = await pool.query<({ total: number } & RowDataPacket)[]>(
...
export async function getLastUpdate(contentType: string) {
    await initDB()
    const [rows] = await pool.query<({ ts: number } & RowDataPacket)[]>(
...
export async function logSiteUpdate(contentType: string, targetName: string, actionType: 'CREATE' | 'UPDATE' | 'DELETE', message: string): Promise<ActionResponse> {
    await initDB()
    ...
    try {
        ...
        const [existing] = await pool.query<SiteUpdate[]>(
