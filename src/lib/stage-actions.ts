...
import { type StageSetup, type Team, type ActionResponse, type Hero } from './types'
...
export async function getStages(type = 'stage') {
    ...
    // Basic listing
    const [rows] = await pool.query<StageSetup[]>(
...
export async function getStageById(id: number) {
    const [rows] = await pool.query<StageSetup[]>('SELECT * FROM stage_setups WHERE id = ?', [id])
    if (rows.length === 0) return null
    const stage = rows[0]

    const [teams] = await pool.query<Team[]>('SELECT * FROM teams WHERE setup_id = ? ORDER BY team_index ASC', [id])
...
import { validateData, StageSetupSchema, type StageSetup as StageSetupInput } from './validation'

export async function createStage(data: StageSetupInput): Promise<ActionResponse> {
    await requireAdmin()
...
export async function updateStage(id: number, data: StageSetupInput): Promise<ActionResponse> {
    await requireAdmin()
...
export async function deleteStage(id: number): Promise<ActionResponse> {
    await requireAdmin()
    try {
...
export async function getAllHeroes() {
    ...
    try {
        ...
        const [dbHeroes] = await pool.query<({ slug: string; name: string; grade: string; hero_group: string; type: string })[] & RowDataPacket[]>("SELECT filename as slug, name, grade, hero_group, type FROM heroes")
...
export async function getHeroProfile(id: string) {
    await initDB()
    const slug = id.replace(/\.[^/.]+$/, "")
    
    // 1. Basic Data
    const [heroRows] = await pool.query<Hero[]>('SELECT * FROM heroes WHERE filename = ?', [slug])
    const hero = heroRows[0] || { filename: slug, name: slug.replace(/_/g, ' '), grade: 'N/A' }
    
    // 2. Builds
    const [buildRows] = await pool.query<RowDataPacket[]>('SELECT * FROM builds WHERE hero_filename = ?', [slug])
    ...
    // Castle Rush
    const [cr] = await pool.query<({ boss_key: string; team_name: string | null; heroes_json: string })[] & RowDataPacket[]>('SELECT boss_key, team_name, heroes_json FROM castle_rush_sets WHERE heroes_json LIKE ?', [pattern])
    cr.forEach(item => usedIn.push({ type: 'Castle Rush', name: item.team_name || item.boss_key, link: `/castle-rush/${item.boss_key}` }))

    // Raid
    const [raid] = await pool.query<({ raid_key: string; heroes_json: string })[] & RowDataPacket[]>('SELECT raid_key, heroes_json FROM raid_sets WHERE heroes_json LIKE ?', [pattern])
    raid.forEach(item => usedIn.push({ type: 'Raid', name: item.raid_key, link: `/raid/${item.raid_key}` }))

    // Dungeon
    const [dungeon] = await pool.query<({ dungeon_key: string; heroes_json: string })[] & RowDataPacket[]>('SELECT dungeon_key, heroes_json FROM dungeon_sets WHERE heroes_json LIKE ?', [pattern])
    dungeon.forEach(item => usedIn.push({ type: 'Dungeon', name: item.dungeon_key, link: `/dungeon/${item.dungeon_key}` }))

    // Advent
    const [advent] = await pool.query<({ boss_key: string; team_name: string | null; team1_heroes_json: string; team2_heroes_json: string })[] & RowDataPacket[]>('SELECT boss_key, team_name, team1_heroes_json, team2_heroes_json FROM advent_expedition_sets WHERE team1_heroes_json LIKE ? OR team2_heroes_json LIKE ?', [pattern, pattern])
    advent.forEach(item => usedIn.push({ type: 'Advent', name: item.team_name || item.boss_key, link: `/advent/${item.boss_key}` }))

    // Arena
    const [arena] = await pool.query<({ heroes_json: string })[] & RowDataPacket[]>('SELECT heroes_json FROM arena_teams WHERE heroes_json LIKE ?', [pattern])
    if (arena.length > 0) usedIn.push({ type: 'Arena', name: 'Recommended Teams', link: '/arena' })

    // Stages
    const [stages] = await pool.query<({ id: number; name: string })[] & RowDataPacket[]>(`
