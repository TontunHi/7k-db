...
export default function HeroStatsBuilderView({ heroes = [], items = [] }) {
    ...
    const finalStats = useMemo(() => {
        ...
        Object.values(equippedItems).forEach(slot => {
            ...
            slot.substats.forEach(sub => {
                if (sub.key) {
                    ...
                    if (statInfo) {
                        const val = Number(statInfo.values[sub.level || 0]) || 0
        ...
        Object.entries(setCounts).forEach(([setName, count]) => {
            ...
            if (numCount >= 2) {
                const bonusesConfig = (SET_BONUSES as any)[setName]
                if (!bonusesConfig) return
                
                const bonus = numCount >= 4 ? bonusesConfig[4] : bonusesConfig[2]
                if (!bonus) return

                Object.entries(bonus).forEach(([statKey, val]) => {
                    const numVal = val as number
                    if (statKey === 'atk_all_perc') extraPerc.atk += numVal
                    else if (statKey === 'def_perc') extraPerc.def += numVal
                    else if (statKey === 'hp_perc') extraPerc.hp += numVal
                    else if (otherStats[statKey] !== undefined) {
                        (otherStats as any)[statKey] += numVal
                    }
                })
            }
        })
