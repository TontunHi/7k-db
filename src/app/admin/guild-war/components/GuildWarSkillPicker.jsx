...
/**
 * GuildWarSkillPicker - Modal for selecting hero skills in rotation
 */
export default function GuildWarSkillPicker({ 
    setIdx, 
    slotIdx, 
    teamHeroes, 
    skillsMap, 
        ...
        <div className={styles.modalOverlay} onClick={onClose}>
            <div className={styles.modalContent} onClick={e => e.stopPropagation()}>
                <header className={styles.modalHeader}>
                    <div>
                        <h3 className="text-xl font-black text-white">Strategic Skill Selection</h3>
                        <p className="text-[10px] font-black uppercase tracking-widest text-indigo-500">ASSIGN ACTION TO SLOT {slotIdx + 1}</p>
                    </div>
                ...
                <div className={styles.modalBody}>
                    ...
                    {!teamHeroes.some(h => h) && (
                        <div className="text-center py-10">
                            <p className="text-sm italic text-muted-foreground font-bold">No combatants assigned. Deploy heroes to the team first.</p>
                        </div>
                    )}
                </div>
            </div>
