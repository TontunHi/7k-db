...
import { Plus, X, Save, Check, Trash } from "lucide-react"
...
export default function CastleRushHeroBuildPicker({ isOpen, onClose, heroFile, initialBuild, items, onSave }) {
                                ...
                                {Array.from({ length: 5 }).map((_, i) => {
                                    ...
                                    return (
                                        <div key={i} className={clsx(
                                            ...
                                            {acc ? <SafeImage src={`/items/accessory/${acc.image}`} alt="" fill className="object-cover" /> : <Plus className="w-4 h-4 text-gray-800" />}
                                        </div>
                                    )
                                })}
                            </div>
                                ...
                                {Array.from({ length: 5 }).map((_, i) => {
                                    ...
                                    return (
                                        <div key={i} onClick={() => openItemSelector(accessories, "refining", i)} className={clsx(
                                            ...
                                            {refinedImg ? <SafeImage src={`/items/accessory/${refinedImg}`} alt="" fill className="object-cover" /> : <Plus className="w-3.5 h-3.5 text-gray-800" />}
                                        </div>
                                    )
                                })}
                            </div>
                                    ...
                                    {selectorItems.map(img => {
                                        ...
                                        return (
                                            <button key={img} onClick={() => handleSelectorClick(img)} className={clsx(
                                                ...
                                                <SafeImage src={`/items/${(selectorTarget.type === 'weapons' ? 'weapon' : selectorTarget.type === 'armors' ? 'armor' : 'accessory')}/${img}`} alt="" fill className="object-cover" />
    ...
    return (
        <div className={clsx("relative rounded-xl p-2.5 flex gap-2.5 items-center border", isWeapon ? "border-red-500/10" : "border-blue-500/10")} style={{ background: "rgba(20,20,20,0.5)" }}>
            <button onClick={onClick} className={clsx("relative w-12 h-12 bg-black rounded-lg border flex-shrink-0 overflow-hidden", isWeapon ? "border-red-500/20" : "border-blue-500/20")}>
                {item.image ? (
                    <SafeImage src={`/items/${type.toLowerCase()}/${item.image}`} alt="" fill className="object-cover" />
                ) : (
