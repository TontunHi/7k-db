...
/**
 * AdventManagerView - Premium Dashboard for Advent Expedition Strategy Management
 */
    ...
    return (
        <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <header className="flex items-center gap-4 pb-6 border-b border-white/5">
                <Marker color="bg-violet-500" className="w-2.5 h-12 rounded-full" />
                <div>
                    <h1 className="text-3xl font-black text-white tracking-tight uppercase italic">
                        Advent <span className="text-violet-500 font-extrabold not-italic">Manager</span>
                    </h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-2">
                {bosses.map((boss) => (
                    <Link
                        ...
                        className="group flex flex-col rounded-2xl overflow-hidden border border-white/5 bg-neutral-950/20 transition-all duration-500 hover:border-violet-500/40 hover:-translate-y-2 hover:shadow-2xl hover:shadow-violet-950/30"
                    >
                        {/* Top: Image Section */}
                        <div className="relative aspect-video w-full overflow-hidden bg-neutral-950 border-b border-white/5">
                            ...
                            <div className="absolute top-4 left-4 z-20">
                                {boss.setCount > 0 ? (
                                    <span className="bg-violet-600 text-white px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-md shadow-violet-600/20 backdrop-blur-md">
                            ...
                            {boss.image && !boss.image.includes('undefined') ? (
                                ...
                                <div className="w-full h-full flex items-center justify-center bg-violet-950/5">
                                    <span className="text-xs font-black opacity-20 italic">No Image</span>
                                </div>
                            )}
                        ...
                        <div className="relative p-5 bg-neutral-900/30 backdrop-blur-md flex flex-col justify-between flex-grow">
                            ...
                            <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-violet-500 to-fuchsia-500 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500" />
                            
                            <div>
                                <span className="text-[9px] font-black uppercase tracking-widest text-violet-400 flex items-center gap-1.5 mb-1.5">
                                    <Sparkles size={10} /> Active Boss
                                </span>
                                
                                <h3 className="text-lg md:text-xl font-black text-white italic tracking-tight uppercase group-hover:text-violet-400 transition-colors duration-300">
                                    {boss.name}
                                </h3>
                            </div>
