...
/**
 * ArenaSkillPicker - Modal for selecting hero skills in rotation
 */
export default function ArenaSkillPicker({ 
    setIdx, 
    slotIdx, 
    teamHeroes, 
    skillsMap, 
        ...
        <div className={styles.modalOverlay} onClick={onClose}>
            <div className={styles.modalContent} onClick={e => e.stopPropagation()}>
                <header className={styles.modalHeader}>
                    <div>
                        <h3 className="text-xl font-black text-white">Tactical Skill Intel</h3>
                        <p className="text-[10px] font-black uppercase tracking-widest text-amber-500/80">SELECT ACTION FOR SLOT {slotIdx + 1}</p>
                    </div>
                ...
                <div className={styles.modalBody}>
                    ...
                    {!teamHeroes.some(h => h) && (
                        <div className="text-center py-10">
                            <p className="text-sm italic text-muted-foreground font-bold">No squads deployed yet. Assign heroes to the team first.</p>
                        </div>
                    )}
                </div>
            </div>
