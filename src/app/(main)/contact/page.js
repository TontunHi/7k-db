...
export default async function ContactPage() {
    ...
    return (
        <div className="container mx-auto px-4 py-20 max-w-4xl font-sans tracking-tight leading-relaxed">
            ...
            <div className="max-w-2xl mx-auto mb-20 animate-in fade-in duration-1000 delay-500">
                {isEnabled ? (
                    <div className="bg-card border border-border rounded-3xl p-8 md:p-12 shadow-2xl relative overflow-hidden">
                        ...
                        <h2 className="text-2xl font-black text-foreground uppercase italic mb-8 flex items-center gap-3">
            ...
            <div className="p-8 rounded-3xl bg-card/40 border border-border shadow-2xl backdrop-blur-sm overflow-hidden relative group">
...
function ContactCard({ icon: Icon, label, value, desc, link }) {
    ...
    return (
        <Component 
            {...extraProps}
            className={cn(
                "p-8 rounded-2xl bg-card border border-border transition-all duration-300 group flex flex-col items-center text-center shadow-sm hover:shadow-xl hover:-translate-y-1 block",
                link ? "hover:border-primary/50 cursor-pointer" : "hover:border-border"
            )}
        >
            <div className="w-12 h-12 rounded-xl bg-background border border-border flex items-center justify-center text-muted-foreground group-hover:text-primary transition-all mb-6 group-hover:shadow-[0_0_20px_rgba(255,215,0,0.2)]">
                <Icon size={24} strokeWidth={1.5} />
            </div>
            <h3 className="text-xs font-black text-muted-foreground uppercase tracking-widest mb-2">{label}</h3>
            <p className="text-lg font-black text-foreground italic transform -skew-x-6 tracking-tight mb-4 group-hover:text-primary transition-colors leading-tight">{value}</p>
            <p className="text-sm text-muted-foreground font-light leading-snug">{desc}</p>
        </Component>
    )
}
