        ...
        <div className="container mx-auto px-4 py-20 max-w-4xl font-sans tracking-tight leading-relaxed">
            <div className="mb-16 text-center relative">
                ...
                <h1 className="text-4xl md:text-6xl font-black text-foreground italic transform -skew-x-12 uppercase mb-6 drop-shadow-lg leading-tight">
                    {lang === 'th' ? "นโยบายความเป็นส่วนตัว" : "PRIVACY "}<span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-amber-500">{lang === 'th' ? "" : "POLICY"}</span>
                </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
                <PolicyCard 
                    icon={ShieldCheck} 
                    title={t("privacy.transparency_title", "Data Transparency")} 
                    color="text-primary"
                    bg="bg-primary/10"
                    border="border-primary/20"
                >
                    {t("privacy.transparency_desc", "7K DB is built for the player experience. We do not require account registration, and we do not collect personal identifying information (PII) from our users. Your privacy is paramount.")}
                </PolicyCard>

                <PolicyCard 
                    icon={EyeOff} 
                    title={t("privacy.analytics_title", "Site Analytics")} 
                    color="text-amber-500"
                    bg="bg-amber-500/10"
                    border="border-amber-500/20"
                >
                    {t("privacy.analytics_desc", "We use minimal, anonymous analytics to understand site traffic and usage patterns. This data helps us optimize the user experience and ensures the site remains fast and accessible for everyone.")}
                </PolicyCard>

                <div className="md:col-span-2">
                    <PolicyCard 
                        icon={LinkIcon} 
                        title={t("privacy.external_title", "External Platforms")} 
                        color="text-orange-500"
                        bg="bg-orange-500/10"
                        border="border-orange-500/20"
                    >
                        {t("privacy.external_desc", "Our site may contain links to external platforms such as YouTube, Discord, or official game communities. We recommend reviewing their respective privacy policies as they are separate from 7K DB.")}
                    </PolicyCard>
                </div>
            </div>
            ...
            <section className="pt-10 border-t border-border/50 text-center">
                <p className="text-muted-foreground/60 text-xs uppercase tracking-widest font-black italic">
                    {t("privacy.last_modified", "Last Modified: April 2026")}
                </p>
            </section>
        </div>
    )
